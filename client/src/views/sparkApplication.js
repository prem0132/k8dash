...
export default class Pod extends Base {
  componentDidMount() {
    ...
    this.registerApi({
      ...
      metrics: api.metrics.pod(namespace, name, metrics =>
...
function getErrors(item) {
  ...
  if (item.status.conditions)
    return item.status.conditions.map(x => x.message).filter(x => !!x);
